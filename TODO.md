...
- Validate every `StructuredOutputContract` schema when the contract is constructed or before the run starts, using a clearly selected JSON Schema draft.
- Centralize schema and payload validation behind the output-contract layer so structured tasks, investigation final outputs, and future structured APIs share identical behavior.
- After parsing provider output, validate the resulting object locally instead of trusting provider-side enforcement alone.
- Return structured validation errors containing contract name, instance path, schema path, and a safe error message. Do not log the full payload because it may contain sensitive data.
- Treat provider enforcement and local validation as complementary guarantees:
  - provider enforcement constrains generation;
  - local validation protects the application boundary.
- Default to explicit failure when output violates the contract. Do not silently repair, coerce, drop fields, or invent missing values.
- Consider a future explicit `retry_once` policy, but keep `fail` as the accuracy-first default and provide the retry only with validation diagnostics and the existing evidence.
- Preserve compatibility for ordinary text outputs. Invalid structured outputs may become newly visible failures, which is an intentional correctness improvement.
- Add tests for invalid schemas, missing required fields, invalid enum values, additional properties, nested paths, provider non-compliance, and sensitive-payload-safe diagnostics.
